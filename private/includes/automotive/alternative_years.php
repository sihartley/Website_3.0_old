...
<section class="alt-years">
    ...
    <?php
        ...
        while ($alt_year = mysqli_fetch_assoc($alt_years)) {
            ...
            $years_alt = $alt_year['year'];
            $year_start = explode(' ', $years_alt)[0];
            $image = $alt_year['image_path'] . $alt_year['vehicle_250px_image'];
            $image_alt = "BUY $make $model $years_alt Vehicle Graphics";
            $href = "/pages/automotive/vehicle-main.php?make=$make&model=$model&year=$year_start";
            $aria_label = "SHOP $years_alt $make $model Vehicle Graphics";
            ?>
            <div class="<?= "vehicle-" . $vehicle++ ?>">
                <div class="content">
                    <h3><?= "$years_alt $make $model" ?></h3>
