...
// Graphics Query
function graphic_trailer_query($table):array {
    global $dbPrefix, $graphics;
    $graphic_columns = [
        'id', 'part_number', 'product_name', 'image_path', /*'product_image_on',*/ 'design_image', 'width_ratio', 'height_ratio', 'size_width_min', 'size_height_max', 'price_base', 'ppsi_base', 'unit', 'weight_base', 'part_graphic'
    ];
    $select_graphics = implode(', ', $graphic_columns);
    $graphic_query = "( SELECT $select_graphics FROM {$dbPrefix}Trailer.$table WHERE available IS NOT NULL )";
...
function containerStart($product_name, $design, $graphic_design):void {
    echo "<div id='cartContainer-$design' class='design cart-container'>"
        . "<h3>$product_name Trailer Graphics</h3>"
...
function graphicSize():void {
    global $graphic, $trailer_sale_discount;
    ...
    foreach (range($graphic['size_width_min'], floor($graphic['size_height_max'] / $graphic['height_ratio']), 3) as $width) {
        $height = number_format($width * $graphic['height_ratio'], 2);

        $price = number_format(((($width * $graphic['height_ratio']) * $width) * $graphic['ppsi_base']) + $graphic['price_base'], 2);
        $price = number_format($price - ($price * $trailer_sale_discount), 2);
        $base_price = number_format(((($graphic['size_width_min'] * $graphic['height_ratio']) * $graphic['size_width_min']) * $graphic['ppsi_base']) + $graphic['price_base'], 2);
        $price_adj = number_format($price - $base_price, 2);

        $weight = number_format($graphic['weight_base'] *
            ((($width * $graphic['height_ratio']) * $width) /
                ($graphic['size_width_min'] * ($graphic['size_width_min'] * $graphic['height_ratio']))), 4);
...
function pricing ():string {
    ...
    $price = ((($graphic['size_width_min']*$graphic['height_ratio'])*$graphic['size_width_min'])*$graphic['ppsi_base'])+$graphic['price_base'];
